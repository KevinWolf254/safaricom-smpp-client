...
public class InBoundSmppSessionStateListener extends AbstractSmppSessionStateListener {
	...
	public InBoundSmppSessionStateListener(String host, int port, BindParameter bindParameter, long bindTimeout,
	...
	public void reconnect() {
		executor.execute(() -> {
			...
			while (getSession() == null || getSession().getSessionState().equals(SessionState.CLOSED)) {
				try {
					log.info("Rescheduling InBoundSmppSession reconnect after {} millis", reconnectInterval);
					Thread.sleep(reconnectInterval);
					log.info("Reconnecting InBoundSmppSession...");
					String systemId = session.connectAndBind(host, port, bindParameter, bindTimeout);
					log.info("Reconnected InBoundSmppSession with systemid {}", systemId);
				} catch (IOException e) {
